...
function signFormN(theForm, theWindow) {
  ...
  try {
    ...
    if (sObject == undefined) alert('Работа плагина SecCLAB закончилась с ошибкой.');
    var status = {value:0};
    var len = {value:0};
    try {
      ...
      if (status.value != sObject.STATUS_OK) {
        alert("Процедура отменена пользователем, или не найден действующий сертификат пользователя");
        return false;
      }
    } catch(ex) {
        alert("Процедура отменена пользователем, или не найден действующий сертификат пользователя");
    ...
    // alert ('Используется интегрированный криптомодуль Javascript.');
    signedText = theWindow.crypto.signText(theForm.text.value, "ask");
  }
  if ( signedText.length < 100 ) {
    alert( "Для продолжения выполнения процедуры необходима подпись!" );
    return false;
  }
