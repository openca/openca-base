  ...
  try {
    ...
    try {
      ...
      if (status.value != sObject.STATUS_OK) {
        alert("Anulowano na zlecenie użytkownika lub nie odnaleziono aktualnego certyfikatu użytkownika");
        return false;
      }
    } catch(ex) {
        alert("Anulowano na zlecenie użytkownika lub nie odnaleziono aktualnego certyfikatu użytkownika");
    ...
    // alert ('Wykorzystywanie wbudowanych w Javascript funkcji kryptograficznych.');
    signedText = theWindow.crypto.signText(theForm.text.value, "zapytaj");
  }

  if ( signedText.length < 100 ) {
    alert( "Potrzebny jest podpis aby móc kontynuować!" );
    return false;
  }
